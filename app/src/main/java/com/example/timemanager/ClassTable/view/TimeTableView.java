package com.example.timemanager.ClassTable.view;
...
public class TimeTableView extends LinearLayout {
    ...
    private Map<Integer, List<Class_Table_Course>> courseMap = new HashMap<>();
     ...
     * @param courses
     */
    public void loadData(List<Class_Table_Course> courses, Date date) {
     ...
     * @param weekNum 周次
     */
    private void handleData(Map<Integer, List<Class_Table_Course>> courseMap, List<Class_Table_Course> courseList, long weekNum) {
        courseMap.clear();
        for (Class_Table_Course c : courseList) {
            ...
            for (int i = 0; i < courseArray.length; i++) {
                Class_Table_Course clone = c.clone();
                ...
                if ("n".equals(info[2]) || weekNum % 2 == 0 && "d".equals(info[2]) || weekNum % 2 == 1 && "s".equals(info[2])) {//非单双周
                    ...
                    List<Class_Table_Course> courses = courseMap.get(clone.getDay());
                    if (null == courses) {
     ...
     * @param weekNum 周次
     */
    private void flushView(Map<Integer, List<Class_Table_Course>> courseMap, long weekNum) {
        //初始化主布局
     ...
     * @param day        星期
     */
    private void addDayCourse(ViewGroup pViewGroup, Map<Integer, List<Class_Table_Course>> courseMap, int day) {
        LinearLayout linearLayout = new LinearLayout(mContext);
        linearLayout.setLayoutParams(new LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1));
        linearLayout.setOrientation(VERTICAL);
        List<Class_Table_Course> courses = getCourses(courseMap, day);
        if (null != courses) {
            for (int i = 0, size = courses.size(); i < size; i++) {
                Class_Table_Course course = courses.get(i);
     ...
     * @return 课程信息List
     */
    public List<Class_Table_Course> getCourses(Map<Integer, List<Class_Table_Course>> courseMap, int day) {
        final List<Class_Table_Course> courses = courseMap.get(day);
        if (null != courses) {
            Collections.sort(courses, new Comparator<Class_Table_Course>() {
                @Override
                public int compare(Class_Table_Course o1, Class_Table_Course o2) {
     ...
     * @param course 课程信息
     */
    private void addCourseCell(ViewGroup pViewGroup, Class_Table_Course course) {
        addHorizontalTableLine(pViewGroup);
